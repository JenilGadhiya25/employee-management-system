...
const Dashboard = () => {
  ...
  const me = attendance[0]
  const myTask = tasks[0]

  // Chart data
  const chartData = [
    { name: 'Mon', value: me?.presentDays || 0 },
    { name: 'Tue', value: me?.presentDays || 0 },
    { name: 'Wed', value: me?.presentDays || 0 },
    { name: 'Thu', value: me?.presentDays || 0 },
    { name: 'Fri', value: me?.presentDays || 0 },
  ]
  ...
  return (
    <div style={{ maxWidth: 1400, padding: '0 20px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28 }}>
        <h1 style={{ fontSize: 28, fontWeight: 800, color: '#0F172A', margin: 0 }}>My Dashboard</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 20, marginBottom: 28 }}>
        {[
          { label: 'Present Days', value: me?.presentDays || 0, trend: '+18%' },
          { label: 'Total Hours', value: `${me?.totalHours || 0}h`, trend: '+11%' },
          { label: 'My Tasks', value: myTask?.total || 0, trend: '+8%' },
        ].map((item, idx) => (
          <div key={idx} style={{
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid #E2E8F0' }}>
                  <th style={{ textAlign: 'left', padding: '12px 0', fontSize: 12, fontWeight: 600, color: '#64748B' }}>Task</th>
                  <th style={{ textAlign: 'left', padding: '12px 0', fontSize: 12, fontWeight: 600, color: '#64748B' }}>Status</th>
                  <th style={{ textAlign: 'left', padding: '12px 0', fontSize: 12, fontWeight: 600, color: '#64748B' }}>Due</th>
                </tr>
              </thead>
              <tbody>
          ...
          <div style={{
            ...
            <div style={{ marginTop: 20, paddingTop: 20, borderTop: '1px solid #E2E8F0' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {[
                  { label: 'Attendance', value: `${me?.presentDays || 0} days` },
                  { label: 'Tasks Done', value: `${myTask?.completed || 0}/${myTask?.total || 0}`, highlight: true },
                ].map((item, idx) => (
                  <div key={idx} style={{
